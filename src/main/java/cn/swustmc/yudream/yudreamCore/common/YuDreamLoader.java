...
public class YuDreamLoader {
    public static void loader() {
        for (Plugin plugin : YudreamCore.instance.getServer().getPluginManager().getPlugins()) {
            ...
            CorePlugin corePlugin = plugin.getClass().getAnnotation(CorePlugin.class);
            if (corePlugin != null) {
                String scanPackage = corePlugin.scanPackage().isEmpty() ? plugin.getClass().getPackageName() : corePlugin.scanPackage();
                YudreamCore.instance.getLogger().info("检测到基于YuDreamCore的插件[" + plugin.getName() + "]即将开始加载!");
                ...
                if (corePlugin.isRegisterCommand()) {
                    try {
                        CommandManager.getInstance().loadCommand((JavaPlugin) plugin, corePlugin.scanPackage());
                    } catch (Exception e) {
                        YudreamCore.instance.getLogger().warning("注册指令出现问题! " + e.getMessage());
                        e.printStackTrace();
                    }
                }
                YudreamCore.instance.getLogger().info("已加载 " + plugin.getName() + " 插件!");
            }
        }
    }
}
